#!/usr/bin/env python3
"""
Matilda Brain - A powerful AI assistant for your terminal
...
class DefaultCommandGroup(click.Group):
    """Route unknown positional input to the configured default command."""
...
@click.version_option('1.1.0', '--version', '-V', prog_name='brain')
...
def cli(ctx, verbose, debug, config):
    ...
    if ctx.invoked_subcommand is None:
        ...
        invoke_hook(
            ctx.obj,
            'on_ask',
            {                'prompt': (),                'model': None,                'temperature': 0.7,                'max_tokens': None,                'tools': False,                'session': None,                'system': None,                'stream': False,                'no_stream': False,                'json': None,            },
        )
...
@click.option('--stream', is_flag=True, default=False,              help="Force streaming output")
@click.option('--no-stream', is_flag=True, default=False,              help="Return the completed response without streaming")
@click.option('--json', is_flag=True, default=None,              help="Output response in JSON format")
@click.pass_obj
def ask(ctx, prompt, model, temperature, max_tokens, tools, session, system, stream, no_stream, json):
    """Quickly ask one-off questions"""
    try:
        kwargs = {            'prompt': prompt,            'model': model,            'temperature': temperature,            'max_tokens': max_tokens,            'tools': tools,            'session': session,            'system': system,            'stream': stream,            'no_stream': no_stream,            'json': json,        }
...
def config_get(ctx, key):
    ...
    try:
        kwargs = {            'key': key,        }
        invoke_hook(ctx, 'on_config_get', kwargs)
    except Exception as e:
        handle_error(e, ctx.verbose)
...
def config_set(ctx, key, value):
    ...
    try:
        ...
        invoke_hook(ctx, 'on_config_set', kwargs)
    except Exception as e:
        handle_error(e, ctx.verbose)
...
def config_list(ctx, show_secrets):
    ...
    try:
        ...
        invoke_hook(ctx, 'on_config_list', kwargs)
    except Exception as e:
        handle_error(e, ctx.verbose)
@cli.group('tools')
@click.pass_obj
...
def tools_enable(ctx, tool_name):
    ...
    try:
        ...
        invoke_hook(ctx, 'on_tools_enable', kwargs)
    except Exception as e:
        handle_error(e, ctx.verbose)
...
def tools_disable(ctx, tool_name):
    ...
    try:
        kwargs = {            'tool_name': tool_name,        }
        invoke_hook(ctx, 'on_tools_disable', kwargs)
    except Exception as e:
        handle_error(e, ctx.verbose)
...
def tools_list(ctx, show_disabled):
    ...
    try:
        ...
        invoke_hook(ctx, 'on_tools_list', kwargs)
    except Exception as e:
        handle_error(e, ctx.verbose)
@cli.command('serve')
@click.option('--host', type=click.STRING, default='127.0.0.1',              help="Host address to bind to")
@click.option('--port', '-p', type=click.INT, default=8772,              help="Port to listen on")
@click.pass_obj
def serve(ctx, host, port):
    """Start the Matilda Brain HTTP server"""
    try:
