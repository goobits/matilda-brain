# Matilda Brain Development Makefile

.PHONY: help test test-unit test-integration test-fast test-coverage lint format format-check type-check quality check clean install dev

PY ?= python3

help: ## Show this help message
	@echo "Matilda Brain Development Commands:"
	@echo "===================================="
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | sort | awk 'BEGIN {FS = ":.*?## "}; {printf "  \033[36m%-15s\033[0m %s\n", $$1, $$2}'

test: ## Run offline unit tests
	@./scripts/test.py

test-unit: ## Run unit tests only (fast, no API calls)
	@./scripts/test.py unit

test-integration: ## Run integration tests with deterministic HTTP mocks
	@./scripts/test.py integration

test-fast: ## Run tests without rate limiting delays
	@BRAIN_RUN_CRED_TESTS=0 REAL_API_TESTS=0 $(PY) -m pytest tests/ -x -q --fast

test-coverage: ## Run the full offline suite with the coverage floor
	@BRAIN_RUN_CRED_TESTS=0 REAL_API_TESTS=0 $(PY) -m pytest tests/ -q --fast --timeout=20 --cov=src/matilda_brain --cov-report=term-missing:skip-covered --cov-fail-under=70

lint: ## Run linting with ruff
	@echo "Running linter..."
	@$(PY) -c "import ruff" 2>/dev/null || (echo "ruff is not installed. Install dev deps: python3 -m pip install -e '.[dev]'"; exit 1)
	@# cli.py is generated by Goobits; lint the source-of-truth instead (goobits.yaml/app_hooks.py).
	@$(PY) -m ruff check src/matilda_brain/ tests/ scripts/test.py examples/ --extend-exclude src/matilda_brain/cli.py

format: ## Format code with black
	@echo "Formatting code..."
	@$(PY) -c "import black" 2>/dev/null || (echo "black is not installed. Install dev deps: python3 -m pip install -e '.[dev]'"; exit 1)
	@# cli.py is generated by Goobits; format the source-of-truth instead (goobits.yaml/app_hooks.py).
	@$(PY) -m black src/matilda_brain/ tests/ scripts/test.py examples/ --line-length 120 --force-exclude 'cli\.py$$'

format-check: ## Check formatting (non-mutating)
	@echo "Checking format..."
	@$(PY) -c "import black" 2>/dev/null || (echo "black is not installed. Install dev deps: python3 -m pip install -e '.[dev]'"; exit 1)
	@# cli.py is generated by Goobits; check source-of-truth formatting instead.
	@$(PY) -m black src/matilda_brain/ tests/ scripts/test.py examples/ --line-length 120 --check --force-exclude 'cli\.py$$'

type-check: ## Run type checking with mypy
	@echo "Running type checker..."
	@$(PY) -c "import mypy" 2>/dev/null || (echo "mypy is not installed. Install dev deps: python3 -m pip install -e '.[dev]'"; exit 1)
	@# cli.py is generated by Goobits; type-check the source-of-truth instead (goobits.yaml/app_hooks.py).
	@$(PY) -m mypy src/matilda_brain/ scripts/test.py examples/ --exclude 'cli\.py$$'

quality: format-check lint type-check ## Run all code quality checks
	@echo "All quality checks completed!"

check: quality test-coverage ## Run the complete offline verification gate

clean: ## Clean up build artifacts and cache
	@echo "Cleaning up..."
	@rm -rf __pycache__ .pytest_cache .mypy_cache .coverage htmlcov
	@find . -type d -name "__pycache__" -exec rm -rf {} + 2>/dev/null || true
	@find . -type f -name "*.pyc" -delete

install: ## Install package with pipx
	@./scripts/setup.sh install

dev: ## Install in development mode
	@./scripts/setup.sh install --dev
